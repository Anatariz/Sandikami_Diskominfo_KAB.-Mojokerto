...
@section('title', 'Edit Katalog Layanan | Sandikami')
@section('page_title', 'Katalog Layanan')

@section('content')
<div class="card mb-4" style="border-left: 4px solid var(--primary);">
    <h2 class="mb-1" style="font-size: 1.25rem;">Edit Layanan: {{ $layanan->nama_layanan }}</h2>
    <p class="text-text-muted mb-0" style="font-size: 0.9rem;"><a href="{{ route('admin.katalog.index') }}" class="text-secondary">Katalog Layanan</a> / Edit</p>
</div>

<form action="{{ route('admin.katalog.update', $layanan->id) }}" method="POST" id="form-builder">
    @csrf
    @method('PUT')
    ...
    <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 30px;">
        ...
        <div>
            <div class="card">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="nama_layanan" name="nama_layanan" class="form-control" required value="{{ $layanan->nama_layanan }}">
                </div>

                <div class="form-group">
                    <label class="form-label" for="deskripsi">Deskripsi Singkat *</label>
                    <textarea id="deskripsi" name="deskripsi" class="form-control" rows="3" required>{{ $layanan->deskripsi }}</textarea>
                </div>

                <div class="form-group">
                    <label class="form-label" for="ikon">Class Icon (RemixIcon)</label>
                    <input type="text" id="ikon" name="ikon" class="form-control" value="{{ $layanan->ikon }}">
        ...
        <div>
            ...
            <div class="card">
                ...
                <input type="hidden" name="form_schema" id="form_schema_input" value="{{ json_encode($layanan->form_schema) }}">
                ...
                <div style="margin-top: 30px; text-align: right; border-top: 1px solid var(--border-color); padding-top: 20px;">
                    ...
                    <button type="submit" class="btn btn-primary"><i class="ri-save-line mr-1"></i> Perbarui Layanan</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initial schema from backend
        let existingSchema = { pemohon: [], layanan: [] };
        try {
            let parsed = JSON.parse(schemaInput.value);
            if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
                existingSchema = parsed;
            } else if (Array.isArray(parsed)) {
                // Backward compatibility if it was an array
                existingSchema.layanan = parsed;
            }
        } catch(e) {
        ...
        // Initialize fields
        if (existingSchema.pemohon && existingSchema.pemohon.length > 0) {
            existingSchema.pemohon.forEach(item => addField(containerPemohon, item));
        } else {
            addField(containerPemohon, {label: 'Nama Lengkap Pemohon', type: 'text', required: true});
        }

        if (existingSchema.layanan && existingSchema.layanan.length > 0) {
            existingSchema.layanan.forEach(item => addField(containerLayanan, item));
        } else {
